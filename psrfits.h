...
#define PSRFITS_MAXFILELEN_SEARCH 500L
#define PSRFITS_MAXFILELEN_FOLD 500L

// The following is the template file to use to create a PSRFITS file.
// Path is relative to GUPPI_DIR environment variable.
#define PSRFITS_SEARCH_TEMPLATE "src/PSRFITS_v3.4_search_template.txt"
#define PSRFITS_SEARCH32_TEMPLATE "src/PSRFITS_v3.4_search32bits_template.txt"
#define PSRFITS_FOLD_TEMPLATE "src/PSRFITS_v3.4_fold_template.txt"
...
struct hdrinfo {
    ...
    char jday[24];          // Julian day and start time (DYYYYMMDDTHHMMSS)
    char beam[24];          // BEAM number
    ...
    double rm;              // RM that each channel was de-rotated at (rad/cm^3)
    ...
    int smjd_start;         // sMJD for sincron start
    int gpu_id;             // GPU id
    ...
    int64_t nsblk;              // Number of spectra per row
...
struct subint {
    ...
    int64_t bytes_per_subint;   // Number of bytes for one row of raw data
...
struct foldinfo {
    ...
    polyco *pc;             // Pointer to polyco blocks
...
struct psrfits {
    ...
    // TODO int psr_id;     // ID of the pulsar
    struct hdrinfo hdr;
    struct subint sub;
