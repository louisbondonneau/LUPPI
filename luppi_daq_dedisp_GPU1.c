...
int main(int argc, char *argv[]) {
   ...
   */
    ...
    if((pfo_fast=fopen(script_fast,"w"))!=NULL) {
        ...
        if (databfdirname[0] != '\0') {
            sprintf(string, "rsync -avt --chmod=o+r,o-wx --remove-source-files --bwlimit=50000 -e \"ssh -T -c aes128-ctr -o Compression=no\"  %s/%s*  nfrplsobs@databfnfrdt:/data/nenufar-pulsar/%.4s/%.4s/%.2s/%s\n", path, basename, projid, year, month, databfdirname); fputs(string,pfo_slow);
        } else {
        ...
        if (databfdirname[0] != '\0') {
            sprintf(string, "rsync -avt --chmod=o+r,o-wx --remove-source-files -e \"ssh -T -c aes128-ctr -o Compression=no\"  %s/%s*  nfrplsobs@databfnfrdt:/data/nenufar-pulsar/%.4s/%.4s/%.2s/%s\n", path, basename, projid, year, month, databfdirname); fputs(string,pfo_fast);
        } else {
