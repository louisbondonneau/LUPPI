...
#define INT64_MAX  1152921504606846975 /* Biggest number that can fit in long long*/
...
#define INT64_MAX  1152921504606846975
const char *hstring;    /* character string containing FITS header information
                   in the format <keyword>= <value> {/ <comment>} */
const char *keyword;    /* character string containing the name of the keyword
...
int64_t *ival;
{
    ...
    if (value != NULL) {
        ...
        dval = atof (val);
        if (dval+0.001 > INT64_MAX)
            *ival = INT64_MAX;
        else if (dval >= 0)
            *ival = (int64_t) (dval + 0.001);
        else if (dval-0.001 < minint)
            *ival = minint;
        else
            *ival = (int64_t) (dval - 0.001);
...
char *str;      /* String (returned) */
{
    ...
    if (hgetc(hstring,keyword) != NULL) {
        lval = strlen (hgetc(hstring,keyword));
        if (lval < lstr)  {
            strcpy(str,hgetc(hstring,keyword));
            strcpy(str,hgetc(hstring,keyword)); //ADD LOUIS
            }
        else if (lstr > 1)  {
            strncpy(str,hgetc(hstring,keyword), lstr-1);
            }
        else  {
            str[0] = value[0];
            }
...
/* In standard FITS, data lasts until 80th character */
    ...
    //printf("debug value_inside line = %s\n",line);fflush(stdout);
