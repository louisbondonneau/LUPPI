...
int psrfits_obs_mode(const char *obs_mode) {
    ...
    if (strncmp("SEARCH", obs_mode, 6)==0) { return(search); }
        ...
        sprintf(strlog, "obs_mode '%s' not recognized, defaulting to SEARCH", obs_mode);
	log_warn("psrfits_obs_mode", strlog);
        return(search);
    }
    return(search);
}
...
int psrfits_create(struct psrfits *pf) {
    ...
    long double ldtmp;
    double dtmp;
    char ctmp[40], strlog[256], fitsmsg[128];
    struct hdrinfo *hdr;
    struct foldinfo *fld;
    ...
    if (mode==fold) {
        if (hdr->onlyI) {
            sprintf(strlog, "In folding mode and ONLY_I is set!");
	    log_warn("psrfits_create", strlog);
	}    
        if (hdr->ds_time_fact > 1) {
            sprintf(strlog, "In folding mode and DS_TIME is > 1!");
	    log_warn("psrfits_create", strlog);
	}    
        if (hdr->ds_freq_fact > 1) {
            sprintf(strlog, "In folding mode and DS_FREQ is > 1!");
	    log_warn("psrfits_create", strlog);
	}    
    }
    ...
    //printf("write_psrfits> filename = %s\n", pf->filename);fflush(stdout);	

    // Create basic FITS file from our template
    // Fold mode template has additional tables (polyco, ephem)
    char *nuppi_dir = getenv("NUPPI_DIR");
    nuppi_dir="/data/nuppi_dir";
    char template_file[1024];
    if (nuppi_dir==NULL) {
        sprintf(strlog, "NUPPI_DIR environment variable not set, exiting.\n");
	log_error("write_psrfits", strlog);
        exit(1);
    }
    //sprintf(nuppi_dir, "/usr/local/src/nuppi-test32bits/nuppi"); // GD
    //printf("Opening file '%s' ", pf->filename);
    if (mode==search) { 
        if (hdr->nbits > 8) sprintf(template_file, "%s/%s", nuppi_dir, PSRFITS_SEARCH32_TEMPLATE);
        else sprintf(template_file, "%s/%s", nuppi_dir, PSRFITS_SEARCH_TEMPLATE);
    } else if (mode==fold) { 
        sprintf(template_file, "%s/%s", nuppi_dir, PSRFITS_FOLD_TEMPLATE);
    }
    printf("Opening template %s \n", template_file);

    pthread_mutex_lock(&lock_psrfits_tpl);
    fits_create_template(&(pf->fptr), pf->filename, template_file, status);
    pthread_mutex_unlock(&lock_psrfits_tpl);

    // Check to see if file was successfully created
    if (*status) {
        fits_get_errstatus(*status, fitsmsg);
        sprintf(strlog, "Error creating psrfits file from template : %s", fitsmsg);
	log_error("write_psrfits", strlog);
    ...
    if (hdr->onlyI || hdr->summed_polns) {
        if (!hdr->onlyI && hdr->npol > 1) {
            sprintf(strlog, "Can't have %d polarizations _and_ be summed!", hdr->npol);
	    log_warn("write_psrfits", strlog);
        }
        itmp = 2;
    ...
    // Update the column sizes for the colums containing arrays
    {
        ...
        if (mode==search) {
	    if (hdr->nbits > 8) itmp = out_nchan * out_npol * out_nsblk; // GD, The PSRFITS Template has a float column, so that we no longer multiply by nbits when recording floats
            else itmp = (hdr->nbits * out_nchan * out_npol * out_nsblk) / 8;
        } else if (mode==fold)
...
int psrfits_write_subint(struct psrfits *pf) {
    ...
    if (pf->filenum==0 || 
        ...
        if (pf->filenum) {
            sprintf(strlog, "Closing file '%s'", pf->filename);
	    log_info("psrfits_write_subint", strlog);
            fits_close_file(pf->fptr, status);
        }
	sprintf(strlog, "Create psrfits file");
	log_info("psrfits_write_subint", strlog);
        psrfits_create(pf);
    }
	...
	if (hdr->nbits > 8) fits_write_col(pf->fptr, TFLOAT, 17, row, 1, out_nbytes/sizeof(float), sub->data, status);
        else fits_write_col(pf->fptr, TBYTE, 17, row, 1, out_nbytes, 
                       sub->data, status);
    } else if (mode==fold) { 
    ...
    if (*status) {
        fits_get_errstatus(*status, fitsmsg);
        sprintf(strlog, "Error writing subint %d : %s", pf->rownum, fitsmsg);
	log_warn("psrfits_write_subint", strlog);
        fits_report_error(stderr, *status);
        fflush(stderr);
    }
    ...
    if (!(*status)) {
        ...
        if (mode==fold && pf->quiet!=1) {
            sprintf(strlog, "Wrote subint %d (total time %.1fs)", pf->rownum-1, pf->T);
	    log_info("psrfits_write_subint", strlog);
            fflush(stdout);
        }
...
int psrfits_write_polycos(struct psrfits *pf, polyco *pc, int npc) {
...
int psrfits_write_ephem(struct psrfits *pf, FILE *parfile) {
    ...
    while (fgets(line, 256, parfile)!=NULL) {
        ...
        if (*status) {
	    fits_get_errstatus(*status, fitsmsg);
	    sprintf(strlog, "psrfits_write_ephem failed, key=%s val=%s : %s", key, val, fitsmsg);
	    log_warn("psrfits_write_ephem", strlog);
            fits_report_error(stderr, *status);
            *status=0;
        } 
#if 0  // DEBUG
...
int psrfits_close(struct psrfits *pf) {
    ...
    if (!pf->status) {
        ...
        sprintf(strlog, "Closing file '%s'", pf->filename);
        log_info("psrfits_close", strlog);
    }
    sprintf(strlog, "Done.  %s %d subints (%f sec) in %d files (status = %d)", pf->mode=='r' ? "Read" : "Wrote", pf->tot_rows, pf->T, pf->filenum, pf->status);
    log_info("psrfits_close", strlog);
    return pf->status;
}
