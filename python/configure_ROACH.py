...
import time, struct, sys, logging, socket, re #corr
...
class ROACH():    
	...
	self.log = log
        try:
            ...
            self.log.info('Connecting to server %s on port %i... '%(roach,katcp_port))
            time.sleep(1)

            if self.fpga.is_connected():
                self.log.info('Connected to server %s on port %i... '%(roach,katcp_port))
            else:
                self.log.error('Failed to connect to server %s on port %i'%(roach,katcp_port))
                self.exit_fail()
    ...
    def load_design(self, boffile):
	    self.boffile = boffile
            ...
            self.log.debug('Clearing the fpga')       
            self.fpga.progdev('')
            time.sleep(1)
            self.log.info('Programming the fpga with %s'%(self.boffile))   
            self.fpga.progdev(self.boffile);
            time.sleep(15)
            self.log.info('Programming complete')
    ...
    def exit_fail(self):
        self.log.error('FAILURE DETECTED in configure_ROACH.py')
        try:
            self.fpga.stop()
        except: pass
        raise
	...
	# N Taps * LCM(reorder orders)*(FFTSize/simultaneousinputs)*numips*packetsize
        ...
        self.log.info('Configuring a sync period of %d'%(sync_period))
    ...
    def init_10gbe_blocks(self):
        ...
        for i in range(0,self.numtengbe):
            self.log.info('Initializing '+gbe_base+str(i))
    ...
    def init_ip_table(self):
        ...
        self.log.info('Initializing ip table')
    ...
    def clear_ip_table(self):
        # zero the ip table
        self.log.info('Zeroing ip table')
        for i in range(0,self.numips):
