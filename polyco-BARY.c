...
int make_polycos(const char *parfile, struct hdrinfo *hdr, char *src,  polyco **pc) {
    ...
    if (fout==NULL) {
        sprintf(strlog, "Error writing to temp dir");
        log_error("polyco", strlog);
        fclose(pf);
        return(-1);
    }
    ...
    // -- Set polyco filename --
    char polycofile[128];
    sprintf(polycofile, "polyco.dat-%d", (int) hdr->fctr);
    ...
    if (mjd1==mjd0) mjd1++;
    sprintf(line, "tempo -z -f pulsar.par -Z PSR=%s -Z SITE=%c -Z OUT=%s -Z START=%f -Z FREQ=%.1lf -Z TOBSH=48 -Z SPAN=60M", parsrc, tcode,  polycofile, mjd0-0.5, hdr->fctr);
    log_info("polyco", line);
    rv = system(line);
    if(rv==-1) {
    ...
    if (pcfile==NULL) {
        sprintf(strlog, "Error reading %s", polycofile);
        log_error("polyco", strlog);
        return(-1);
    }
