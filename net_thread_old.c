/**********************************************
*version 1.01
*04/02/2020 stt_offs = 0.0;   ligne 171 j ai commente cette ligne car elle imposait un stt_offs de 0 a la place de seq_num*dt
...
void *net_thread(void *_args) {
 ...
 setniceness(READNICENESS);
  ...
  while(run) {
      ...
      if( (first == 1) && (one == 2) ) {last_seq_num = seq_num - nbof_subpackets;}
      if( (first == 1) && (one == 2) ) {last_time_stamp = time_stamp;
		one=0;}
      if( (first == 1) && (one == 1) ) {
		one = 2;
		goto initialization;}
      ...
      force_new_block=0;
      //printf("seq_num_diff=%lld\n",seq_num_diff);
      //printf("nbof_subpackets=%llu\n",nbof_subpackets);
      ...
      if ( ((int64_t)time_stamp_diff == (int64_t)0) && (first != 1) ) {
		...
		last_time_stamp = time_stamp;
      	...
      	else if (first == 1)  //first time_stamp jump --> START
      	{
            ...
            if( (stt_smjd == smjd_start) || (smjd_start < 0) )
            {
      		if ( seq_num_diff+195312 == nbof_subpackets ) {
      			seq_num_diff += 195312;
      	...
      	//sprintf(strlog, "Waiting for the first packet (seq_num=%"PRIu64", last_seq_num=%"PRIu64", diff=%"PRIu64"  time_stamp_diff=%"PRIu64")", seq_num, last_seq_num, seq_num_diff, time_stamp_diff);
	//printf("W");
      	last_seq_num = seq_num;
	last_time_stamp = time_stamp;
      	continue;
      }
      else { // probably lost 1 or more time_stamp
      	sprintf(strlog, "4Received out-of-order packet probably lost more than one time_stamp (seq_num=%"PRIu64", last_seq_num=%"PRIu64", diff=%"PRIu64"  time_stamp_diff=%"PRIu64")", seq_num, last_seq_num, seq_num_diff, time_stamp_diff);
      	if ( time_stamp_diff % 2 == 1 ) {
      		if ( paire == 0 ) { paire=1; }
		else { paire=0; }
      	}

      	log_warn("net_thread", strlog);
      	npacket_total += (int)((float)seq_num_diff/(float)nbof_subpackets); //+ (int)(((floor(((float)time_stamp_diff-1.)*195312.5))+paire)/nbof_subpackets);  // a corrig
      	ndropped_total += (int)((float)seq_num_diff/(float)nbof_subpackets) - 1;
      	cont += (int)((float)seq_num_diff/(float)nbof_subpackets);
      	last_seq_num = seq_num;
	last_time_stamp = time_stamp;
      ...
      /*
      if (seq_num==0) { force_new_block=1;}
      if (seq_num_diff<=0) {
          printf("INSIDE\n");
          sprintf(strlog, "Start signal ? (seq_num = %"PRIu64"  seq_num_diff = %"PRId64"  IP = %"PRIu64")", seq_num, (int64_t)seq_num_diff, IP_id);
          log_info("net_thread", strlog);
