...
void init_fold(dedispersion_setup *s) {
    ...
    if ((s->fft_len - s->overlap) % 64) {
        sprintf(strlog, "nvalid=%d does not divide evenly into threads_per_block=%d", 
                s->fft_len - s->overlap, s->fold_thread_per_block);
        log_warn("init_fold", strlog);
    }
...
int compute_fold_params(dedispersion_setup *s, const polyco *pc) {
    ...
    //printf("rf=%.2lf  bw=%.2lf  dm=%.2lf  earth_z4=%.2lf\n", s->rf, s->bw, s->dm, s->earth_z4);
    //printf("imjd=%d  fmjd=%.2lf  nbins_fold=%d \n", s->imjd, s->fmjd, s->nbins_fold );
...
void zero_fold_buffers(dedispersion_setup *s) {
 ...
 */
extern "C"
void fold(dedispersion_setup *s, int chan, foldbuf *fb) {
    /* Sizes */
    const unsigned npol = 4;  // Assume always full-Stokes for now..
    const size_t bytes_per_profile = sizeof(float) * npol * s->nbins_fold;
    const size_t bytes_per_count = sizeof(unsigned) * s->nbins_fold;
